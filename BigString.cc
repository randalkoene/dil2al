...
// returns >0 if a>b (also happens if b ends earlier than a)
// returns <0 if a<b (also happens if a ends earlier than b)
// returns 0 if a==b
{
  if (b == 0)
    return *a != 0;
...
// Hold that thought... the two lines below may not be true...
  // Update 20200425: C++ no longer permits default value in friend declaration
  // that isn't also definition, hence the definition was moved into BigString.hh.
...
{
  // BEWARE: The sbumpc() function deals weirdly with input that consists of
  // empty lines. As a consequence, you can't return from this readline()
  // function when reading from cin after just pressing ENTER.
  // It is still better to use getline() for that.
#if (__GNUC__ >= 3)
  istream::sentry ipfx(s);
  if (!ipfx) return 0;
#else
